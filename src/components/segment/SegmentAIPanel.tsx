...
function _ResultRow({ result }: { result: SegmentResult }) {
  const scoreColor =
    result.score >= 0.9
      ? '#22c55e'
      : result.score >= 0.7
...
interface SegmentAIPanelProps {
  ...
  /** Whether the P3-SAM service has available connection slots */
  isConnectionAvailable?: boolean;
  /** Number of remaining connection slots (0–2) */
  remainingSlots?: number;
  /** Whether the active asset is already marked as segmented (enables Smart Organize) */
...
export default function SegmentAIPanel({
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── CTA / Progress ───────────────────────────────────────────────────── */}
      {/* Note: Errors are displayed in NavActions panel via job store for consistency */}
      <div
      ...
      >
        ...
        <button
          onClick={handleStart}
          disabled={isSegmenting || !isEnabled || !isConnectionAvailable}
          className={cn(
            'w-full py-3 rounded-xl text-sm font-bold text-[#1a1a2e] transition-opacity hover:opacity-90 disabled:opacity-60 btn-gradient',
        ...
        >
          ...
          <span className="btn-content">
            {isSegmenting ? (
              ...
              <span>✨ Submit Segmentation · {remainingSlots} available</span>
            )}
          </span>
        </button>
      ...
      {/* ── Smart Organize ─────────────────────────────────────────────── */}
      {/* Show when: active asset is already segmented (loaded from store or GLB extras) */}
      {isAssetSegmented && !isSegmenting && onSmartOrganize && (
        <div
        ...
        >
          <button
            ...
            className="w-full py-2.5 rounded-xl text-sm font-bold text-[#1a1a2e] transition-opacity hover:opacity-90 disabled:opacity-60 btn-gradient"
          >
            {/* Layered gradients for smooth crossfade */}
            <span className="gradient-bg bg1" style={{ background: 'linear-gradient(135deg, #7c3aed, #D5B451)' }} aria-hidden />
            <span
              className={`gradient-bg bg2${isOrganizing ? ' animate' : ''}`}
              style={{ background: 'linear-gradient(135deg, #D5B451, #7c3aed)' }}
              aria-hidden
            />

            <span className="btn-content text-white">
              {isOrganizing ? (
                <>
                  <span className="spinner" aria-hidden />
