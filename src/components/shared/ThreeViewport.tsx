...
const getAssetUrl = (path: string) => {
  // In WC mode: staticBase = CDN origin (e.g. "http://cdn/phidias-static")
  // In Standalone mode: staticBase is null → assets served by Next.js from /public
  const staticBase = usePhidiasStore.getState().staticBase || '';
...
function GLBModel({
  ...
  // Track whether we're in color mode
  const isColorMode = segmentColors && Object.keys(segmentColors).length > 0;

  useEffect(() => {
    if (!clonedScene) return;

    if (isColorMode) {
      // COLOR MODE: Apply segment colors to all meshes
      clonedScene.traverse((child) => {
        if (!(child instanceof THREE.Mesh)) return;
        const oid = objectId(child);

        // Save original material ONLY the first time we enter color mode
        // Only save if it's a valid Three.js material (not already colored)
        if (!child.userData.__origMaterialSaved && child.material) {
          const mat = child.material;
          // Only save if it's a proper Three.js material (has constructor name)
          if (mat.constructor && mat.constructor.name !== 'Object' &&
              (mat instanceof THREE.Material || mat.constructor.name.includes('Material'))) {
            child.userData.__origMaterial = mat;
            child.userData.__origMaterialSaved = true;
          } else {
            // Mark as saved anyway to avoid re-checking
            child.userData.__origMaterialSaved = true;
          }
        }

        const segColor = segmentColors?.[oid] ?? segmentColors?.[child.name];
        if (segColor) {
          // No highlight effect - just apply segment color
      ...
      // ORIGINAL MODE: Restore original materials and apply renderMode
      clonedScene.traverse((child) => {
        if (!(child instanceof THREE.Mesh)) return;

        // Clone the material so each mesh has its own instance - otherwise multiple meshes
        // sharing the same material will all get the same highlight effect
        // First try to restore from saved original (if we previously entered color mode)
        const saved = child.userData.__origMaterial;
        if (saved && saved instanceof THREE.Material) {
          child.material = saved.clone();
        } else if (child.material && child.material instanceof THREE.Material) {
          // No saved material - clone the current one to ensure isolation
          child.material = child.material.clone();
        }

        // Apply renderMode - ALWAYS ensure visible
        switch (renderMode) {
          case 'wireframe': {
          ...
          case 'textured':
          default: {
            // For textured mode: ensure we have a visible MeshStandardMaterial
            if (child.material instanceof THREE.MeshStandardMaterial) {
              child.material.wireframe = false;
              child.material.emissive = new THREE.Color(0);
              child.material.emissiveIntensity = 0;
            } else if (child.material instanceof THREE.MeshBasicMaterial) {
              // If we have a basic material (e.g., no texture), create a standard one with color
              const color = child.material.color ? '#' + child.material.color.getHexString() : '#cccccc';
              child.material = new THREE.MeshStandardMaterial({
                color,
                roughness: 0.5,
                metalness: 0,
              ...
              // Fallback: create a basic MeshStandardMaterial so it's visible
              child.material = new THREE.MeshStandardMaterial({
                color: '#cccccc',
                roughness: 0.5,
                metalness: 0,
        ...
        // Apply highlight effect for selected objects (blue outline via emissive)
        const oid = objectId(child);
        const isSelected = selectedObjectId === oid || (selectedObjectIds?.includes(oid) ?? false);

        // Clear highlight first, then apply if selected
        if (child.material instanceof THREE.MeshStandardMaterial) {
          if (isSelected) {
            child.material.emissive = new THREE.Color(0x6366f1);
            child.material.emissiveIntensity = 0.3;
          } else {
            // Clear previous highlight - but preserve if in color mode with segment color
            const hasSegmentColor = segmentColors?.[oid] ?? segmentColors?.[child.name];
            if (!hasSegmentColor) {
              child.material.emissive = new THREE.Color(0);
              child.material.emissiveIntensity = 0;
            }
  ...
  }, [clonedScene, segmentColors, renderMode, selectedObjectIds, isColorMode, selectedObjectId]);
  ...
  // Render the scene with click handler
  return (
    <group ref={groupRef}>
      <primitive
        object={clonedScene}
...
const DEFAULT_HDR_PATH = '/hdri/qwantani_moon_noon_puresky_4k.hdr';
...
function SceneLighting({ hdrUrl, envIntensity = 1.2, envRotation = 0 }: {
  ...
  const envFile = hdrUrl || getAssetUrl(DEFAULT_HDR_PATH);
  ...
  useEffect(() => {
    ...
    const capture = async () => {
      ...
      const hdrTexture = await new Promise<THREE.DataTexture>((resolve, reject) => {
        rgbeLoader.load(getAssetUrl(DEFAULT_HDR_PATH), resolve, undefined, reject);
      });
      if (cancelled) return;
      ...
      if ('environmentIntensity' in thumbScene) {
        (thumbScene as unknown as Record<string, unknown>).environmentIntensity = 2.0;
      }
      ...
      const _ambient = new THREE.AmbientLight(0xffffff, 0.8);
