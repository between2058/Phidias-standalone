...
import { detectGlbMetadata, downloadAssetGlb, triggerDownload } from '@/lib/three/loaders';
import { validateGLBFile } from '@/lib/three/glb-validator';
...
type ModelTypeFilter = 'all' | 'textured' | 'untextured' | 'rigged' | 'splat' | 'segmented' | 'organized';
...
function StatusBadge({ status, progress }: { status?: Asset['status']; progress?: number }) {
  ...
  if (status === 'segmenting') {
    return (
      ...
      >
        <div className="w-2 h-2 rounded-full border border-[#a78bfa] border-t-transparent animate-spin" />
        {progress !== undefined ? `${Math.round(progress)}%` : 'Seg…'}
      </div>
    );
  }
  if (status === 'organizing') {
    return (
      ...
      >
        <div className="w-2 h-2 rounded-full border border-[#22c55e] border-t-transparent animate-spin" />
        Org…
      </div>
    );
  }
...
const TYPE_GRADIENTS: Record<string, string> = {
  ...
  segmented: 'linear-gradient(145deg, #1a0a2e 0%, #2d1b4e 50%, #4a2d7a 100%)',
  organized: 'linear-gradient(145deg, #0a2e1a 0%, #1b4e2d 50%, #2d7a4a 100%)',
};

const TYPE_ICONS: Record<string, string> = {
  textured: '🎨', untextured: '🧊', rigged: '🦾', splat: '✦', segmented: '🔍', organized: '✨',
};

// ─── Asset Badge System ───────────────────────────────────────────────────────
//
// Instead of chaining `asset.foo && !asset.bar && !asset.baz`, we declare a
// priority-ordered list of badge descriptors.  getAssetBadge() picks the
// highest-priority entry whose validate() returns true and renders that one.
// This makes it trivial to add new badges or change precedence without
// touching multiple JSX branches.

interface AssetBadgeDescriptor {
  /** Higher = shown first when multiple conditions are true */
  priority: number;
  /** Returns true when this badge should be shown for the given asset */
  validate: (asset: Asset) => boolean;
  /** Emoji / short label shown in the badge */
  label: string;
  /** Tooltip text */
  title: (asset: Asset) => string;
  /** Tailwind-compatible inline styles for the pill */
  style: React.CSSProperties;
}

const ASSET_BADGES: AssetBadgeDescriptor[] = [
  {
    priority: 30,
    validate: (a) => !!a.isOrganized,
    label: '✨ organized',
    title: (a) => `Smart Organize applied${a.segmentation ? ` · ${a.segmentation.numParts} parts` : ''}`,
    style: { background: 'rgba(34,197,94,0.3)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.5)' },
  },
  {
    priority: 20,
    validate: (a) => !!a.segmentation,
    label: '',  // rendered dynamically below
    title: (a) => `Segmented into ${a.segmentation?.numParts ?? '?'} parts`,
    style: { background: 'rgba(139,92,246,0.3)', color: '#a78bfa', border: '1px solid rgba(139,92,246,0.5)' },
  },
  {
    priority: 10,
    validate: (a) => !!a.hasSkinnedMesh,
    label: '🦴 Armature',
...
/** Return the single highest-priority badge that applies to an asset, or null. */
function getAssetBadge(asset: Asset): AssetBadgeDescriptor | null {
  let best: AssetBadgeDescriptor | null = null;
  for (const b of ASSET_BADGES) {
    if (b.validate(asset) && (best === null || b.priority > best.priority)) {
      best = b;
    }
  }
  return best;
...
/** Render the winning badge pill for a card (absolute-positioned). */
function AssetBadge({ asset }: { asset: Asset }) {
  const badge = getAssetBadge(asset);
  if (!badge) return null;
  // Special case: segmentation badge shows numParts inline
  const label =
    badge.priority === 20
      ? `🔍 ${asset.segmentation?.numParts ?? '?'}`
      : badge.label;
  return (
    <div
...
/** Render the winning badge as an inline pill (for list rows). */
function AssetBadgeInline({ asset }: { asset: Asset }) {
  const badge = getAssetBadge(asset);
  if (!badge) return null;
  const label =
    badge.priority === 20
      ? `🔍 ${asset.segmentation?.numParts ?? '?'} parts`
      : badge.label;
  return (
    <span
...
function AssetContextMenu({
  ...
  onUploadToPegaverse: (id: string) => void;
  ...
  const items = [
    ...
    ...(hostApp === 'pegaverse' ? [{ label: 'Upload to Pegaverse', icon: '↑', action: () => { onUploadToPegaverse(menu.assetId); onClose(); } }] : []),
    null, // divider
...
function FilterDropdown({
  ...
  const MODEL_TYPES: { value: ModelTypeFilter; label: string; Icon: LucideIcon; icon?: string }[] = [
    ...
    { value: 'segmented', label: 'Segmented', Icon: Layers, icon: '🔍' },
    { value: 'organized', label: 'Organized', Icon: Sparkles, icon: '✨' },
  ];
...
function AssetCard({
  ...
  return (
    ...
    >
      ...
      {/* Badge: highest-priority applicable badge (organized > segmented > armature) */}
      {!isManageMode && <AssetBadge asset={asset} />}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          {/* Badge pill (single highest-priority badge) */}
          <AssetBadgeInline asset={asset} />
...
export default function AssetsPanel({ defaultTab = 'assets' }: { defaultTab?: PanelTab }) {
  ...
  const [uploadModalAssetId, setUploadModalAssetId] = useState<string | null>(
    null,
  );
  ...
  // Automatically switch tabs based on route
  useEffect(() => {
    if (pathname?.includes('/workspace/segment')) {
      setPanelTab('scene');
    } else if (pathname?.includes('/workspace/model') || pathname?.includes('/workspace/image')) {
      setPanelTab('assets');
    }
  }, [pathname]);
  ...
  useEffect(() => {
    const isSegmentMode = pathname?.includes('/workspace/segment');
    if (!sceneGraph && !segmentHierarchy && panelTab === 'scene' && !isSegmentMode) {
      setPanelTab('assets');
    }
  }, [sceneGraph, segmentHierarchy, panelTab, pathname]);
  ...
  const handleBatchExport = () => {
    ...
    for (const asset of selected) {
      downloadAssetGlb(asset).catch(() => {
        // Fallback: raw URL download (uses triggerDownload for shadow DOM compatibility)
        triggerDownload(asset.modelUrl!, `${asset.name.replace(/\.[^.]+$/, "")}.glb`);
      });
    }
    exitManageMode();
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setActiveAssetId(id);
    e.target.value = '';
    // Async: read Phidias metadata from the GLB and restore badge/type
    const lowerName = file.name.toLowerCase();
    if (lowerName.endsWith('.glb') || lowerName.endsWith('.gltf')) {
      detectGlbMetadata(url).then((meta) => { if (meta) updateAsset(id, meta); });
    }
  };
  ...
  const handleContextDownload = (id: string) => {
    ...
    downloadAssetGlb(asset).catch((err) => {
      console.error('[AssetsPanel] Download failed, falling back to raw URL:', err);
      triggerDownload(asset.modelUrl!, `${asset.name}.glb`);
    });
  };
  ...
  const handleContextUploadToPegaverse = (id: string) => {
    setUploadModalAssetId(id);
  };
  ...
  const tabs: TabDef[] = [
    ...
    ...(sceneGraph || segmentHierarchy || pathname?.includes('/workspace/segment')
    ...
    >
      ...
      {panelTab === 'assets' && (
        <>
          ...
          {uploadError && (
            <div
              className="flex-shrink-0 flex items-center gap-2 px-3 py-2 mx-2 mt-2 rounded-lg"
              style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)' }}
            >
              <AlertCircle size={14} style={{ color: '#f87171' }} />
              <span className="flex-1 text-xs" style={{ color: '#f87171' }}>{uploadError}</span>
              <button
      ...
      {contextMenu && (
        <AssetContextMenu
          ...
          onUploadToPegaverse={handleContextUploadToPegaverse}
          onSetActive={handleContextSetActive}
          onRename={handleContextRename}
        />
      )}

      {/* Upload to Pegaverse Modal */}
      {uploadModalAssetId && (
        <UploadToPegaverseModal
          asset={assets.find((a) => a.id === uploadModalAssetId)!}
          onClose={() => setUploadModalAssetId(null)}
        />
      )}
