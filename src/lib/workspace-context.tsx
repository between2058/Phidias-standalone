...
export interface Asset {
  ...
  thumbnail?: string;
  modelUrl: string;
  type: 'textured' | 'untextured' | 'rigged' | 'splat' | 'segmented' | 'organized';
  isFavorite: boolean;
  createdAt: Date;
  faces?: number;
  vertices?: number;
  // Status & pipeline metadata
  status?: 'ready' | 'generating' | 'failed' | 'queued' | 'segmenting' | 'organizing';
  ...
  // Segmentation workflow tracking
  segmentation?: {
    numParts: number;
    segmentedAt: string; // ISO date string
    params?: {
      pointNum: number;
      promptNum: number;
      threshold: number;
  ...
  /** True when Smart Organize has been applied to this asset */
  isOrganized?: boolean;
  organizedAt?: string; // ISO date string
  /** Job ID from which this asset was generated (for linking NavActions to existing assets) */
  jobId?: string;
}
...
export interface SceneGraphSlot {
  ...
  /** Rename a node in the scene hierarchy */
